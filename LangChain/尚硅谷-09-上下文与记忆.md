...
力。比如：
![尚硅谷-09-上下文与记忆 第1页图片1](./images/尚硅谷-09-上下文与记忆_p001_img01.png)
...
你叫小明。
**我们希望的智能体应该是右图的情况：**
![尚硅谷-09-上下文与记忆 第4页图片1](./images/尚硅谷-09-上下文与记忆_p004_img01.png)
...
类比：
![尚硅谷-09-上下文与记忆 第4页图片2](./images/尚硅谷-09-上下文与记忆_p004_img02.png)


**举例：**
![尚硅谷-09-上下文与记忆 第5页图片1](./images/尚硅谷-09-上下文与记忆_p005_img01.png)
...
![尚硅谷-09-上下文与记忆 第6页图片1](./images/尚硅谷-09-上下文与记忆_p006_img01.png)

#### 1.3.2 记忆的管理
在LangChain v0.x版本中，通过专用的xxxMemory类管理记忆。
在LangChain v1.x版本中，Agent是构建在LangGraph图结构之上的，通过上文提到的state和store构
建记忆系统。使用更简单、功能更统一。
...
输出：略
**说明：**第三轮对话中的 "我刚才问了什么问题？" 就能在第二轮存储的消息历史中找到答案。所有消息
历史都会自动追加到 AgentState 的 messages 字段中，无需手动维护。
**步骤3：更新线程ID**
...
其实我们在课程开始的requirements.txt中已经提供了相关依赖，此处不必也不能重新安装，否则版本
可能冲突。
...
**trim_messages**
**步 触发动作 & 触发前 State 中的 最终传给 LLM 的**
**是否触发与执行**
**骤 传入消息 messages 列表 (长度) messages 实际内容**
**逻辑**
invoke("你
[H: 你好，我是老王] 不触发 (长度
好，我是老 [H: 你好，我是老王]
(1) )
王")
模型自动回 [H: 你好... , A1: 你好 存储到 Memory
-- -
复 (A1) 老王...] (2) 中，等待下次
invoke("从
[H: 你好..., A1: ..., H: 不触发 (长度 [H: 你好..., A1: ..., H: 从
现在起，你
从现在起...] (3) ) 现在起...]
叫小王")
[H: 你好..., A1: ..., H:
...
复 (A3) 中
A3 (共 6 条)
invoke("告 [H: 你好..., A1: ..., H: 触发裁剪！ 长度 [H: 你好，我是老王]
诉我，你是 从现在..., A2: ..., H: 今 7 是奇数 取后 (第一条) [A2, H: 今

谁？我是 天..., A3: ..., H: 告诉 4 条 天..., A3, H: 告诉我...]
谁？") 我...] (7) ( messages[-4:] ) (后4条)
#### 2.4.2 消息删除
...
对象删掉，而是把这个 RemoveMessage 作为一条新记录追加到当前线程的状态历史中。这个
...
#### 2.4.3 摘要
**把早期历史压缩成摘要，再替换原始消息。**
![尚硅谷-09-上下文与记忆 第30页图片1](./images/尚硅谷-09-上下文与记忆_p030_img01.png)
...
![尚硅谷-09-上下文与记忆 第38页图片1](./images/尚硅谷-09-上下文与记忆_p038_img01.png)

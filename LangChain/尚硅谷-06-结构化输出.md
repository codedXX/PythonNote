...
![尚硅谷-06-结构化输出 第17页图片1](./images/尚硅谷-06-结构化输出_p017_img01.png)
...
Schema 作为 Tools 传入。
大模型侧的约束： 像 OpenAI 的 strict=True 参数，会启动模型的语法采样约束（Grammar-
based sampling）。大模型在解码生成 token 时，不是瞎猜，而是严格按照 JSON Schema 的语
法树进行选择，从而在模型底层级保证了输出格式绝不走样。
**第4步：自动解析与验证**
...
3. 返回（Return）： 如果通过验证，你拿到的不再是冷冰冰的字符串，而是一个直接可以点出属性
...
![尚硅谷-06-结构化输出 第19页图片1](./images/尚硅谷-06-结构化输出_p019_img01.png)
...
director 和 rating 的信息都是缺失的，但是前者被标记为必填，因此输出的字典包含该字段但是值为

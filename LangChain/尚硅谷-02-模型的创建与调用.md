...
![尚硅谷-02-模型的创建与调用 第1页图片1](./images/尚硅谷-02-模型的创建与调用_p001_img01.png)
...
阿里云百炼：所有新用户可获得超过5000万Tokens的免费额度及4500张图片生成额度。适合toB
企业用户
...
OpenRouter支持支付宝或微信充值，最低限额$5，税费$0.8，此外，如果调用的模型禁止在国内
使用，如ChatGPT，则无法通过OpenRouter直接调用（需要Magic），会提示This model is not
...
![尚硅谷-02-模型的创建与调用 第3页图片1](./images/尚硅谷-02-模型的创建与调用_p003_img01.png)

**说明：**课程中的部分章节会单独列出相关依赖，主要是为了帮助大家了解该章节涉及的核心库。 无需重
复安装 ，这些依赖已经统一包含在 requirements.txt 中。
## 2、模型初始化角度1：使用模型提供商库
在 LangChain 中初始化模型，主要可以通过直接使用特定的Model Class和使用统一的
...
![尚硅谷-02-模型的创建与调用 第3页图片2](./images/尚硅谷-02-模型的创建与调用_p003_img02.png)
...
**补充说明：账户充值**
![尚硅谷-02-模型的创建与调用 第10页图片1](./images/尚硅谷-02-模型的创建与调用_p010_img01.png)
...
图示：
![尚硅谷-02-模型的创建与调用 第11页图片1](./images/尚硅谷-02-模型的创建与调用_p011_img01.png)
...
![尚硅谷-02-模型的创建与调用 第14页图片1](./images/尚硅谷-02-模型的创建与调用_p014_img01.png)
...
使用的特定提供商的模型名称(必需)。
model str 无
比如：openai:gpt-4o、groq:gemma2-9b-it
model_provider str 模型提供商名称 无
API 密钥。如果不提供，会从环境变量中读取（如
api_key str None
DEEPSEEK_API_KEY ）
base_url str 大模型供应商API请求地址。 None
控制输出随机性，范围 0.0-2.0，温度越高输出越随
机。
temperature float - 0.0 ：最确定性，输出几乎不变 0.7
- 1.0 ：平衡创造性和一致性
- 2.0 ：最随机，最有创造性
max_tokens int 限制模型输出的最大 token 数量 None
timeout float 超时时间（秒），超时未响应，请求会被取消。 None
请求失败（如网络问题、速率限制）时的最大重试次
max_retries int 6
数
**说明：**
...
![尚硅谷-02-模型的创建与调用 第15页图片1](./images/尚硅谷-02-模型的创建与调用_p015_img01.png)
...
![尚硅谷-02-模型的创建与调用 第20页图片1](./images/尚硅谷-02-模型的创建与调用_p020_img01.png)

### 4.2 Ollama的下载-安装
Ollama项目支持跨平台部署，目前已兼容Mac、Linux和Windows操作系统。
无论使用哪个操作系统，Ollama项目的安装过程都设计得非常简单。
访问 https://ollama.com/download 下载对应系统的安装文件。
![尚硅谷-02-模型的创建与调用 第20页图片2](./images/尚硅谷-02-模型的创建与调用_p020_img02.png)
...
![尚硅谷-02-模型的创建与调用 第21页图片1](./images/尚硅谷-02-模型的创建与调用_p021_img01.png)

在文件路径上输入cmd 回车后会自动打开命令行窗口：
![尚硅谷-02-模型的创建与调用 第21页图片2](./images/尚硅谷-02-模型的创建与调用_p021_img02.png)
...
![尚硅谷-02-模型的创建与调用 第21页图片3](./images/尚硅谷-02-模型的创建与调用_p021_img03.png)

然后Ollama就会进入安装，点击install后，可以看到Ollama的安装路径就变成了我们指定的目录了。
### 4.3 模型的下载
**1、手动设置大模型存储目录：**
![尚硅谷-02-模型的创建与调用 第21页图片4](./images/尚硅谷-02-模型的创建与调用_p021_img04.png)


指明大模型要下载到的本地目录位置
![尚硅谷-02-模型的创建与调用 第22页图片1](./images/尚硅谷-02-模型的创建与调用_p022_img01.png)

**2、模型的下载**
方式1：直接在图形化界面位置下载
![尚硅谷-02-模型的创建与调用 第22页图片2](./images/尚硅谷-02-模型的创建与调用_p022_img02.png)
...
![尚硅谷-02-模型的创建与调用 第23页图片1](./images/尚硅谷-02-模型的创建与调用_p023_img01.png)

回到命令行，输入指令开始下载：
![尚硅谷-02-模型的创建与调用 第23页图片2](./images/尚硅谷-02-模型的创建与调用_p023_img02.png)

下载完即可进行交互：
![尚硅谷-02-模型的创建与调用 第23页图片3](./images/尚硅谷-02-模型的创建与调用_p023_img03.png)
...
**角色 英文 作用 示例**
"你是一个专业的 Python 导
system System 设定 AI 的行为、角色、规则
师"
user Human/User 用户的输入/问题 "什么是装饰器？"
AI 的历史回复（用于对话上下
assistant AI/Assistant "装饰器是一种设计模式..."
文）
...
![尚硅谷-02-模型的创建与调用 第43页图片1](./images/尚硅谷-02-模型的创建与调用_p043_img01.png)
...
![尚硅谷-02-模型的创建与调用 第49页图片1](./images/尚硅谷-02-模型的创建与调用_p049_img01.png)
...
这是LangChain针对模型的能力画像，但是否存在，取决于LangChain在集成模型厂商的服务时是否声
明了能力画像。
...
![尚硅谷-02-模型的创建与调用 第57页图片1](./images/尚硅谷-02-模型的创建与调用_p057_img01.png)
...
![尚硅谷-02-模型的创建与调用 第58页图片1](./images/尚硅谷-02-模型的创建与调用_p058_img01.png)
...
字段，用于控制是否启用思考模式。
![尚硅谷-02-模型的创建与调用 第60页图片1](./images/尚硅谷-02-模型的创建与调用_p060_img01.png)
...
**配置项 类型 描述**
为当前运行设置一个可读的名称。如在
run_name str LangSmith追踪系统中快速定位和识别不同的
运行任务。
为运行设置标签，用于分类和过滤。如在
tags List[str] LangSmith追踪系统中快速定位和识别不同的
运行任务。
设置回调处理器，在运行的不同阶段（开始、
List
callbacks 流输出、结束等）触发。与一些监控平台（如
[BaseCallbackHandler]
LangSmith）集成进行深度追踪和调试。
附加任意的键值对元数据。记录本次调用的业
metadata Dict[str,Any] 务上下文，如{"user_id": "123", "session_id":
"abc"}
限制当前可运行对象的最大并发运行数。防止
max_concurrency int 对API接口或本地资源造成过大压力，实现简单
的速率限制。
限制运行时递归调用的最大深度。主要在复杂
recursion_limit int 的工作流（如Agent执行多步工具调用）中，
防止出现无限递归循环。
一个万能字典，用于传递其他可配置参数。实
configurable Dict[Str,Any] 现更高级的动态行为，如配置可替代的模型或
组件。
说明如下：

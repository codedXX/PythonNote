...
* `中间件(Middleware) `：Agent的“中枢”，在执行流程的关键节点进行`拦截、控制和增强`。
...
### 1.1 什么是中间件
**Middleware(中间件)，简单说就是Agent** **执行过程中的`钩子函数`，是** **LangChain** **1.x** **的“`王牌`”工程化**
**能力。**
...
（hooks），让你在`不改 Agent 主体逻辑的情况下实现策略与治理`。
...
这些需求有一个共同特点：`它们不是 Agent的核心业务逻辑，但又会影响Agent的执行过程`。如果把这些逻辑全部直接写进主流程，**会带来几个问题：**
...
LangChain提供的**和模型供应商无关**的内置中间件分为`六个类别`
...
> ## SESSION INTENT
> 用户（老王）与AI（小王）进行初次问候和介绍。没有明确的后续任务目标，会话目前处于社交
> 开场阶段。
> ## SUMMARY
> 用户自称“老王”，向AI问好并询问AI的身份。AI回应，自我介绍为“小王”。用户随后表示很高兴
...
> ==================================•[1m Ai Message
> •[0m==================================
> “你高兴得太早了”是一句常见的网络调侃用语，字面意思是“别急着开心，后面可能有变化或挑
...
> Here is a summary of the conversation to date:
> 对历史消息的摘要如下：
> 用户先自我介绍为“老王”，询问助手是谁；助手回应自己是“小王”。随后用户表示很高兴认识助
> 手。
> ================================== Ai Message
> ==================================
> 你高兴得太早了
...
> ================================== Ai Message
> ==================================
> 哎呀老王，别误会！我的意思是，作为“小王”，跟您这位“老王”比，我的“高兴”确实还“太早”呢
...
- `redact` ：将检测到的PII信息用字符串` [REDACTED_[PII_TYPE]] `替换，其中的 PII_TYPE是上面提
  ...
  除/隐藏” `真实内容。适合日志清洗、合规需求、公开输出时隐藏敏感内容。

- `mask` ：用 *** 将PII信息的前面一部分信息遮蔽。比如信用卡号可能变成 ****-****-

  ****-1234 （只保留最后几位/部分可见），邮箱可能保留域名部分 + 隐藏用户名的一部分等 — 既
...
> ================================== Ai Message
> ==================================
> 抱歉，我不能直接代你发送邮件、查询银行卡余额、访问外部链接，或确认/处理这类敏感标识信
> 息。
> 不过我可以帮你做这些安全替代操作：
> 1. **帮你写邮件内容**
...
> ================================== Ai Message
> ==================================
> 我不能帮助判断或验证一个具体的 API Key 哈希，也不能协助拨打被遮挡的电话号码或访问本
...
比如，当一个大任务需要被拆解为 **3** **个以上的子任务**，且前面的步骤是后面步骤的前提时，如果不列Todo 列表，大模型在执行到第 3 步时，很容易**忘记自己最初的目标**，或者在工具返回大量报错信息“应激”，直接跳过验证去回答用户。
...
> ⭐️⭐️如果把普通的 Agent 比作“想到哪写到哪”的实习生，那么引入了 TodoListMiddleware 的 Agent
>
...
1. pytest会扫描目录下所有以 `test_ 开头`或以` _test` 结尾的文件，视为测试文件
...
**注意：**服务端代码逻辑是80%概率输出非法响应，所以不一定会导致单次请求的工具调用超过限制，**尝试几次**即可看到效果。
...
| ----------- | ------------------------ | -------------------------------------- | ----------------------------------- |
| 第 1 次重试 | `1.0 × 2⁰ = 1.0`         | 严格等于 1.0 秒                        | 在 0～1.0 秒之间随机                |
| 第 2 次重试 | `1.0 × 2¹ = 2.0`         | 严格等于 2.0 秒                        | 在 0～2.0 秒之间随机                |
| 第 3 次重试 | `1.0 × 2² = 4.0`         | 严格等于 4.0 秒                        | 在 0～4.0 秒之间随机                |
| 第 4 次重试 | `1.0 × 2³ = 8.0`         | 严格等于 8.0 秒                        | 在 0～8.0 秒之间随机                |
| 第 5 次重试 | `1.0 × 2⁴ = 16.0 → 10.0` | 严格等于 10.0 秒（受限于 `max_delay`） | 在 0～10.0 秒之间随机               |
| 第 6 次重试 | `1.0 × 2⁵ = 32.0 → 10.0` | 严格等于 10.0 秒（受限于 `max_delay`） | 在 0～10.0 秒之间随机               |
...
* 没有 Jitter 的惨剧（` jitter=False` ）：
  ...
  这意味着，在第 1 秒、第 3 秒、第 7 秒的那个**精准的时间点**上，这 1000 个请求会**整整齐齐地再次****同时轰炸服务**。刚刚复活的服务器瞬间又被这波整齐的峰值流量压垮，形成恶性循环。**
* 引入 Jitter 的优势（` jitter=True` ）：
  通过给重试时间加上随机性，这 1000 个请求会在 秒、 秒的区间内**均匀地错开（削峰填谷）**。流量被平摊到了整条时间轴上，服务器就能轻松地分批处理完这些请求。
...
# 【核心防御】：防止无限循环重跳（死循环）
# 检查消息历史中是否已经注入过这条特殊的系统提示。如果有，说明已经重试过了，不再
重复干预。
...
return response
@wrap_model_call
def wrap_model_middleware2(request: ModelRequest,handler: Callable[[ModelRequest], ModelResponse])
-> ModelResponse | None:

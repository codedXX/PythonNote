...
- **Checkpoint**：某个执行时刻的“状态快照”。里面可能包含聊天消息、节点输出、下一步要执行的节点等。
- **Checkpointer**：负责保存和读取这些快照的组件，相当于“存档系统”。可以存到内存、SQLite、PostgreSQL 等。
- **`thread_id`**：会话的唯一标识，相当于“存档槽编号”。
...
反之，如果**重新执行完整代码**，导致 **`checkpointer = InMemorySaver()`** 被重新执行，那么之前保存在内存中的检查点也会被`清空`。
...
> ⭐**为什么执行完代码后会多出checkpoint_migrations、checkpoints、checkpoint_blobs、checkpoint_writes这四张表？**
> 
>
> 是这行代码创建/迁移截图中的四张表：
>
...
> ```
>
> 它会执行 `langgraph-checkpoint-postgres` 自带的数据库迁移，创建：
>
> | 表                      | 用途                                     |
> | ----------------------- | ---------------------------------------- |
> | `checkpoint_migrations` | 记录已经执行的数据库迁移版本             |
> | `checkpoints`           | 保存每个 `thread_id` 的状态快照及元数据  |
> | `checkpoint_blobs`      | 保存各状态通道序列化后的实际数据         |
> | `checkpoint_writes`     | 保存节点执行过程中的中间写入和待处理写入 |
...
**`LangGraph`** 支持**三种持久化模式**，采用**不同的检查点保存时机，在容灾能力和性能开销、响应时效性之间作取舍**。
...
> ⭐**下面代码里，写不写durability="async"有什么区别吗？**
>
> * 有区别，但在你这段代码中，**显式写 `durability="async"` 和不写通常没有区别**，因为当前 LangGraph `默认`就是 `"async"`。
...
上述代码中，可以通过修改 **`durability`** 参数**切换持久化模式**：
...
**`LangGraph`** 在配置检查点存储器后，会把同一个 **`thread_id`** 下的执行过程保存为一组检查点。通过这些检查点，我们**可以查看图运行的中间状态，也可以为后续的检查点回溯（重放、分叉）和失败恢复做准备**。
...
> ⭐**所以用数据库的话，就没必要加`get_state_history`  ，用内存的话才需要加`get_state_history`  ，是吗？**
>
>
> 不是哦，**数据库和内存都不需要靠 `get_state_history()` 来保存数据。**
>
> 无论用哪一种：
>
> - **只想正常运行、记住之前的数据**：不用加 `get_state_history()`。
> - **想查看之前每一步保存了什么**：才加 `get_state_history()`。
>
> 就像你把文件存到电脑硬盘或者 U 盘，文件都已经存好了。`get_state_history()` 相当于“打开文件夹看看”，不会决定文件有没有保存。
>
> 你第二段用内存的代码，把 `get_state_history()` 和打印历史的代码删掉，**仍然会自动保存，只是程序关闭后内存里的数据会消失。**

> ⭐**如果用数据库的话，重新执行这个程序，这个存档是新的还是会有以前的，用内存的话呢？用一句话总结？**
> 
>
> 用数据库时，同一数据库、同一 `thread_id` 会保留旧存档并增加新存档；用内存时，只有复用原来的 `InMemorySaver` 才保留旧存档，重新创建或重启程序就从头开始。
...
> ~~~
>
> * `("users",)：`末尾的逗号表示这是一个只有一个元素的元组，而不是普通字符串。
> * `*USERS_NS : `表示把 `USERS_NS` 中的元素展开到新元组中。因此最终结果是：namespace1 == ("users", "Alice")
> * `类型标注部分：`
>   - `Final`：告诉类型检查器 `USERS_NS` 不应该被重新赋值，但运行时不会强制禁止。
>   - `Tuple[str]`：表示一个只包含一个字符串元素的固定长度元组。
>
> 
>
> ⭐**store表是哪行代码创建的？**
...
>
> ~~~
> from_conn_string() → 连接数据库
> setup()            → 创建 Store 所需表结构
> put()              → 向表中写入记忆数据
> search()           → 查询记忆数据
> ~~~
...
1. **`namespace`** 使用**元组是硬性约束**，天然表达层级结构。本例使用两层命名空间 **`("users", "Alice")`**，清晰区分了**领域（`users`）**和**实体（用户名）**，而具体存储什么类型的数据则由 **`key`** 参数表达（如 **`"preferences"`**）。
...
4. 如果希望支持语义检索，需要在 **`Store`** 中配置索引和 **`embedding`** 函数。未配置索引时，**`search()`** 只能`按照命名空间和过滤条件检索`。

### 7.2.2. 访问长期记忆

`长期记忆存储器`可以通过节点函数中的 **`runtime.store`** 访问。
...
> ⭐**store.get(namespace, key) 就是去查找数据库里prefix为namespace，key为key的那条数据**
...
⭐这正是`短期记忆和长期记忆协作的典型模式：`

> 第一次调用从长期记忆中读取数据，并写入短期记忆；后续同一会话线程内优先复用短期记忆，避免重复查询长期记忆。



> ⭐**短期记忆也要存到数据库，那为什么不叫长期记忆？**
>
> 
>
> 因为这里的**“短期”和“长期”主要按记忆的使用范围区分，不是按保存时间区分**。存进数据库，是为了持久化，避免程序重启后丢失。
>
> 以 Alice 的两次聊天为例：
>
> - **短期记忆**：会话 `"345"` 里聊过什么、当前任务进行到哪一步。换一个新的 `thread_id`，不会自动带入这些状态。
> - **长期记忆**：Alice 喜欢低糖、原味酸奶。只要新会话也用 `("users", "Alice")` 查询，就能读取这些偏好。
>
> |                | 短期记忆        | 长期记忆            |
> | -------------- | --------------- | ------------------- |
> | 使用范围       | 同一会话内      | 可以跨会话          |
> | 你的存储组件   | `PostgresSaver` | `PostgresStore`     |
> | 定位方式       | `thread_id`     | `namespace` + `key` |
> | 能否存入数据库 | 能              | 能                  |
>
> 所以，**数据库解决“数据能否持久保存”，短期／长期解决“数据在哪些会话中使用”**。
>
> 你的短期记忆即使保存一年，也仍然是某个会话的状态；长期记忆则需要你的代码主动读取，才能用于新的会话。



> ⭐**那PostgresSaver和PostgresStore有什么区别，是存储方式不同吗还是怎么样？分别对应哪些表？**
>
> 
>
> 两者都使用 PostgreSQL，但保存的数据结构、读写方式和用途不同。** `PostgresSaver` 负责保存图的运行状态；`PostgresStore` 负责保存你主动管理的业务数据，例如用户偏好。
>
> | 对比       | `PostgresSaver`             | `PostgresStore`                  |
> | ---------- | --------------------------- | -------------------------------- |
> | 用途       | 会话记忆、运行恢复          | 跨会话的用户偏好等数据           |
> | 保存什么   | 图的状态和执行进度          | 自定义的 JSON 数据               |
> | 谁负责读写 | 配置后由 LangGraph 自动管理 | 你的代码调用 `put()`、`get()` 等 |
> | 如何定位   | `thread_id`、检查点编号等   | `namespace` + `key`              |
> | 接入位置   | `compile(checkpointer=...)` | `compile(store=...)`             |
>
> 你截图里的 **6 张表，前 4 张属于 `PostgresSaver`，后 2 张属于 `PostgresStore`**。
>
> **`PostgresSaver` 对应：**
>
> | 表名                    | 作用                                                         |
> | ----------------------- | ------------------------------------------------------------ |
> | `checkpoints`           | 保存检查点信息，例如所属会话、前一个检查点、状态版本和元数据 |
> | `checkpoint_blobs`      | 保存序列化后的状态字段值，例如消息列表等，按字段和版本组织   |
> | `checkpoint_writes`     | 保存节点执行产生的中间写入结果，支持失败恢复等机制           |
> | `checkpoint_migrations` | 记录数据库表结构的升级版本，不保存聊天内容                   |
>
> 因此，一份会话状态可能需要结合多张表恢复，并非所有消息都直接放在 `checkpoints` 表里。[官方检查点表定义](https://github.com/langchain-ai/langgraph/blob/main/libs/checkpoint-postgres/langgraph/checkpoint/postgres/base.py)
>
> **`PostgresStore` 对应：**
>
> | 表名               | 作用                                                  |
> | ------------------ | ----------------------------------------------------- |
> | `store`            | 保存实际业务数据，包括命名空间、键、JSON 值和时间信息 |
> | `store_migrations` | 记录 Store 表结构的升级版本                           |
>
> 例如：
>
> ```
...
> ```
>
> 数据写入 `store` 表；调用下面的代码就从中读取：
>
> ```
> store.get(("users", "Alice"), "preferences")
> ```
>
> 命名空间在该实现中对应数据库的 `prefix` 字段，业务内容保存在 `value` 字段。[官方 Store 表定义](https://github.com/langchain-ai/langgraph/blob/main/libs/checkpoint-postgres/langgraph/store/postgres/base.py)
>
> 结合你的程序，数据流是：
>
> ```
...
>         │ runtime.store.get(...)
>         ▼
> 节点返回 {"preferences": run_item.value}
>         │
>         ▼
> 偏好进入当前会话的 state
>         │
>         │ LangGraph 通过 PostgresSaver 自动保存
>         ▼
> checkpoint 相关表中的会话检查点
> ```
>
> 所以，**同一份偏好可以同时存在于长期存储和会话状态中**。但它们不会自动同步：更新 `store` 中的偏好后，你当前的路由如果仍跳过查询，就会继续使用会话状态里的旧偏好。
